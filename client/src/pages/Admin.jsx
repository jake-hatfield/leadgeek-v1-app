...
const AdminItem = ({ title, desc, path, color, action }) => {
	return (
		<article
			v-for='item in items'
				...
				user.role === 'admin' ? (
					<section className='my-6'>
						...
						{popup && (
							<div
								ref={wrapperRef}
								className='relative z-20 max-w-lg mt-6 mx-auto p-4 rounded-lg bg-white shadow-lg'
							>
								<h2 className='text-xl font-bold text-gray-800'>
									Upload confirmation required
								</h2>
								<p className='pt-2 pb-6 text-gray-700'>
									Please confirm the leads you're adding aren't duplicates and
